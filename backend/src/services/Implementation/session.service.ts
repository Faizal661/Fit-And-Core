...
export default class SessionService implements ISessionService {
  ...
  async getTrainerAvailability(trainerId: string): Promise<IAvailabilityModel[]> {
    try {
  ...
  private generateSlots(availability: IAvailabilityModel): Partial<ISlotModel>[] {
    ...
    // Generate slots
    for (let currentMinute = startMinutes; currentMinute < endMinutes; currentMinute += slotDuration) {
      const slotStartTime = this.convertMinutesToTime(currentMinute);
      const slotEndTime = this.convertMinutesToTime(currentMinute + slotDuration);
